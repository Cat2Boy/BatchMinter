...
contract BatchMinterV1 {
	...
	function createAndClaim(uint256 _n, uint256 term) external {
		...
		// for (uint256 i = n; i < n+_n; i++) {
			
		// 	bytes32 salt = keccak256(abi.encodePacked(msg.sender, i));
		// 	assembly {
		// 		proxy := create2(0, add(miniProxy, 0x20), mload(miniProxy), salt)
		// 	}
		...
		for(uint i=n; i<n+_n; i++) {
			...
			console.log("contract===", proxy);
			BatchMinterV1(proxy).claimRank(term);
		}
	...
	function claimMintReward(uint256 _n) external {
		for (uint i=n; i<n+_n; i++) {
			...
			BatchMinterV1(proxy).claimMintRewardAndShare(tx.origin, 100);
